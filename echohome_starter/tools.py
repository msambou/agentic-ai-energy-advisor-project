...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    possible_conditions = ["sunny", "partly_cloudy", "cloudy", "rainy"]

    def generate_hourly(day_offset: int) -> List[Dict[str, Any]]:
        hourly_data = []
        ...
        for hour in range(24):
            ...
            solar_irradiance = (
                round(random.uniform(200, 1000), 2) if is_daytime and condition != "cloudy" else 0
            )
    ...
    for day in range(days):
        forecast["hourly"].extend(generate_hourly(day))
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    base_rate = round(random.uniform(0.08, 0.15), 3)  # base off-peak rate per kWh
    ...
    for hour in range(24):
        ...
        if period == "peak":
            rate = round(base_rate * peak_multiplier + random.uniform(-0.01, 0.01), 3)
            demand_charge = round(random.uniform(5, 15), 2)  # flat demand charge (mocked)
        else:
            rate = round(base_rate + random.uniform(-0.005, 0.005), 3)
            demand_charge = 0.0
